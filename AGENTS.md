...
- **계층 경계 (엄격 헥사고날)**: `interfaces → application ← infrastructure`, `application → domain`. `domain` 은 스프링/JPA/Repository 를 모른다.
- **Repository = outbound port**: `application.<usecase>.port` 에 인터페이스, `infrastructure.<aggregate>` 에 구현.
- **트랜잭션 경계 & 조율**: `application.Facade` 가 Repository 호출과 `@Transactional` 을 단일 소유. Repository/I/O 에 의존하는 Domain Service 는 두지 않는다.
...
## 자주 쓰는 패턴
- 신규 도메인 추가: 4계층을 추가한다.
  - `domain.<aggregate>` — 순수 모델/값객체/도메인 규칙
  - `application.<usecase>` — `Facade`, `command/`, `result/`, `port/Repository(I)`
  - `infrastructure.<aggregate>` — `Entity`, `JpaRepository`, `RepositoryImpl(: port.Repository)`
...
## 절대 금지
- `apps/*` 끼리의 직접 의존.
- `domain` 에서 스프링/JPA 어노테이션 또는 Repository 인터페이스 참조.
- `domain` 에 Repository/I/O 에 의존하는 Service 신설 (그런 조율은 Application 으로). 순수 헬퍼(정책/계산기) 는 가능.

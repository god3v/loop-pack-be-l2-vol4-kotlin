...
- 쿠폰 **사용(UC-9)** 은 `CouponFacade.applyCoupon(...)` 진입점 + 도메인 능력(`UserCoupon.use`, `Coupon.calculateDiscount`) 까지만. **OrderFacade 연동·결제 실패 보상은 범위 밖**(order 도메인 소유).
...
### 1.4 Coupon 생성·계산·만료 (`Coupon`)
- [x] `Coupon.create(name, discount, minOrderAmount?, expiredAt, now)` 로 템플릿이 생성된다 (deletedAt=null)
- [x] `expiredAt` 이 `now` 이전(과거) 이면 `COUPON_BAD_REQUEST` 예외가 발생한다
...
- [x] `isExpired(at)` 는 `at` 이 `expiredAt` 이후면 true, 같거나 이전이면 false

### 1.5 Coupon 수정·삭제
- [x] `update(name, discount, minOrderAmount?, expiredAt, now)` 로 필드가 갱신된다
- [x] `update` 도 만료 시각 과거·음수 minOrderAmount 를 거부한다
...
### 1.7 UserCoupon 발급·사용·노출상태 (`UserCoupon`)
- [x] `UserCoupon.issue(userId, couponId)` 로 발급된다 — status=`AVAILABLE`, usedAt=null, issuedAt 설정
- [x] `use(at)` 호출 시 status=`USED`, usedAt=at 로 전이된다
- [x] 이미 `USED` 인 쿠폰의 `use(at)` 는 `ALREADY_USED_COUPON` 예외가 발생한다
- [x] `viewStatus(coupon, at)` 가 `USED` 면 `EXPIRED` 시각이어도 `USED` 를 반환한다 (사용 우선)
- [x] `viewStatus(coupon, at)` 가 `AVAILABLE` + 만료 경과면 `EXPIRED` 를 반환한다
- [x] `viewStatus(coupon, at)` 가 `AVAILABLE` + 만료 전이면 `AVAILABLE` 을 반환한다
...
- [x] 유효 템플릿으로 발급하면 `AVAILABLE` UserCoupon 이 저장되고 `IssuedCouponResult` 가 반환된다
- [x] 템플릿이 없거나 삭제됨 → `COUPON_NOT_FOUND`
- [x] 템플릿이 만료됨 → `COUPON_NOT_APPLICABLE`
...
- [x] 만료 시각 과거 / 잘못된 할인 값(도메인 검증 위임) → `COUPON_BAD_REQUEST`

#### UC-6. `updateCoupon(couponId, command)` — 관리자 수정
- [x] 정상 입력으로 name/discount/minOrderAmount/expiredAt 가 갱신된다
- [x] 미존재/삭제 → `COUPON_NOT_FOUND`
...
#### UC-9. `applyCoupon(userId, userCouponId, orderAmount)` — 쿠폰 사용 (할인 + 소진)
- [x] 정상: 할인 금액을 반환하고 UserCoupon 이 `USED` 로 저장된다
- [x] 발급 쿠폰 미존재 또는 소유자 불일치 → `USER_COUPON_NOT_FOUND`
- [x] 이미 사용된 쿠폰 → `ALREADY_USED_COUPON`
- [x] 템플릿 만료 → `COUPON_NOT_APPLICABLE`
- [x] 최소 주문 금액 미달 → `COUPON_NOT_APPLICABLE`
...
**Coupon**
- [x] save 후 findById 로 동일 도메인 객체가 복원된다 (discount/minOrderAmount/expiredAt 보존)
...
**UserCoupon**
- [x] save 후 findById 로 동일 도메인 객체가 복원된다 (status/usedAt 보존)
...
- [x] 존재 → `200 OK` + `{ id, name, type, value, minOrderAmount, expiredAt }`
- [x] 미존재/삭제 → `404 COUPON_NOT_FOUND`

### 7.3 UC-5 등록 — `POST /api-admin/v1/coupons`
- [x] 정상 등록 → `200 OK` + 생성 템플릿
- [x] 잘못된 `type`/`value`/만료 과거 → `400 COUPON_BAD_REQUEST`
...
- [x] `CouponFacade` 주입. `placeOrder` 에서 `couponId != null` 이면 `couponFacade.applyCoupon(userId, couponId, order.originalAmount)` 호출 후 `order.applyCoupon(...)` (같은 `@Transactional` — REQUIRED 전파)
...
- [x] **동시성 단일 사용(HIGH)**: 사용 경로에 발급 쿠폰 행 **비관적 쓰기 락**(`UserCouponRepository.findByIdForUpdate` = `@Lock(PESSIMISTIC_WRITE)`) 도입 — `CouponFacade.applyCoupon` 이 이를 사용해 동시 주문을 직렬화. `OrderCouponIntegrationTest` 에 N-스레드(CountDownLatch) 동시 주문 → 정확히 1건 성공·1회 소진·재고 1회 차감 검증 추가
